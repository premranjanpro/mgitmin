        ...
        base.init = function () {
            ...
            // DOM manipulations
                ...
                if ( base.options.createPagination ) {
                    ...
                    // Create Pagination
                    for ( var i = 0; i < base.slideCount; i++ ) {
						// Dennis added feature so that thumbnail is shown as slider controllers
						// simply number the thumbnails for a particular slider
						var thmb=i+1;
                        $('<li />', {
                            'data-index': i,
                            text: base.options.paginationTitle ? base.$slides.eq( i ).attr('data-title') : i + 1
                        }).css({ "background-image" : "url('slides/thumbs/" + base.options.thmbPath + thmb + ".gif')" }).appendTo(base.$sliderControls);
                    }; // Pagination appended
